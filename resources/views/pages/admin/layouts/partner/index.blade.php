...
@section('content')
                ...
                <table class="table">
                    ...
                    <tbody class="table-border-bottom-0">
                        @php
                            $no = 1;
                        @endphp
                        @forelse ($partner as $items)
                            <tr>
                                <td>{{ $no++ }}</td>
                                <td>
                                    @if (!empty($items->image_partner))
                                        <img src="{{ asset('images/' . $items->image_partner) }}" alt="Image Team"
                                            class="img-fluid" style="max-width: 50px;">
                                    @else
                                        -
                                    @endif
                                </td>
                                <td>{{ $items->name_partner ?? '-' }}</td>
                                <td>
                                    <button type="button" class="btn btn-sm btn-success" data-bs-toggle="modal"
                                        data-bs-target="#modalEdit{{ $items->id }}">Edit</button>
                                    <form action="{{ route('dashboard.partner.destroy', $items->id) }}" method="POST"
    ...
    <!-- Modal -->
    @include('pages.admin.layouts.partner.modals')
@endsection
