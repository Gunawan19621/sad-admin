...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <div class="row">
                <div class="col-6">
                    <h5 class="card-header">Image Experience</h5>
                </div>
                ...
                <table class="table">
                    ...
                    <tbody class="table-border-bottom-0">
                        @php
                            $no = 1;
                        @endphp
                        @forelse ($imageExperience as $items)
                            <tr>
                                <td>{{ $no++ }}</td>
                                <td>{{ $items->title_experience ?? '-' }}</td>
                                <td>
                                    @if (!empty($items->image_experience))
                                        <img src="{{ asset('images/' . $items->image_experience) }}" alt="Image Story"
    ...
    <!-- Modal -->
    @include('pages.admin.layouts.experience.image-experience.modals')
@endsection
